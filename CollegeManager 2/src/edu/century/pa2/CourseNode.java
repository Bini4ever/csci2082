...
public class CourseNode implements Cloneable
{
	...
	//Modification method to add a new node after this node.   
		...
		public static CourseNode listCopy(CourseNode head) {
			CourseNode position = head;
			CourseNode newHead;
			CourseNode end = null;
			
			newHead = new CourseNode((Course) (position.data).clone(), null);
			end = newHead;
			position = position.link;
			
			while (position != null)
			{
				end.link = new CourseNode((Course) (position.data).clone(), null);
				end = end.link;
				position = position.link;
			}
